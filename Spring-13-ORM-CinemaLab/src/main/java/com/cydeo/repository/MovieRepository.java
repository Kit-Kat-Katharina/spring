...
public interface MovieRepository extends JpaRepository<Movie,Long> {
    ...
    List<Movie> findAllByPriceBetween(BigDecimal priceStart,BigDecimal priceEnds);

    //Write a derived query to list all movies where duration exists in the specific list of duration
    List<Movie>findAllByDurationIn(List<Integer> durations);

    //Write a derived query to list all movies with higher than a specific release date
    List<Movie>findAllByReleaseDateAfterThan(LocalDate releaseDate);
    ...
    List<Movie>retrieveByPriceRange(BigDecimal priceStart, BigDecimal priceEnd);

    //Write a native query to return all movies where duration exists in the range of duration
@Query(value = "SELECT * FROM movie WHERE durantion IN ?1")
