...
public interface TicketRepository extends JpaRepository<Ticket,Long> {
   ...
   Integer countAllByUserAccountIdField(String id);
    ...
    List<Ticket>findAllByDateTime(LocalDateTime dateTime1, LocalDateTime dateTime2);
    ...
    List<Integer>countAllTicketsByUser(Long userId);
    ...
    Integer countAllTicketsByUserBetweenRangeOfDateTime(Long userId,LocalDateTime dateTime1, LocalDateTime dateTime2);


    //Write a native query to distinct all tickets by movie name
    @Query(value = "SELECT DISTINCT (m.name) FROM t JOIN movie_cinema mc ON t.movie_cinema_id = mc.id " + " JOIN movie m ON mc.movie_id = m.id", nativeQuery = true)
    ...
    //Write a native query to list all tickets where a specific value should be containable in the username or accountname or movie name
